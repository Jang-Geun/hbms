function [did] = interp_Npts_master_func(fn, fnz, sid, A, dtmin, basename, basenameout, fileindx, Tf)
%%
%%	input		fn		= filename with ROMS velocity fields
%%			fnz		= filename with ROMS Zeta fields
%%			sid		= 1] use depth averaged vel.   else] surface currents
%%			A		= diffusivity m^2/s   e.g., 1 for high diffusivity
%%			dtmin		= time increment (step) in minutes, e.g. 0.5
%%			basename	= base name for points files (without _#.nc)
%%			basenameout	= base name for output trajectory file
%%			fileindex	= file number for multiple point files
%%			Tf		= time in hours for continuously feeding particles [0 = all at t=0, no feed];
...
outfile = [ basenameout '_' num2str(fileindx)  '_cont_feed_traj.nc'];
outpath = '../image_code/';
...
dn0 = datenum(2016, 1, 1, 0, 0, 0);
dnuvs = t(1)/3600/24 + dn0;
...
dnS = datenum(2017,11,6,11,0,0);
dnE = datenum(2017,11,6,23,0,0);
...
TT(1) = floor((dnS-dnuvs)*24*3600) + t(1); %number of seconds from begining 
TTs = TT(1);
%%dnt = dnuvs+t/3600/24;
...
if Tf > 0,
	Tf = Tf*60;  %% feed duration in minutes
	Nfinc = Tf/dtmin;
	Nper = floor(N_pts/Nfinc);
end;
...
for i = 1:ntimes
	...
	if (i < Nfinc & Tf > 0),   %%% reset points incrementally until past feed period
		X(kk+1, tfind) = X(kk, tfind);
		Y(kk+1, tfind) = Y(kk, tfind);
	   	X(kk, tfind) = 0;
	   	Y(kk, tfind) = 0;
	end;	
	if mod(i,wsize) == 0,
