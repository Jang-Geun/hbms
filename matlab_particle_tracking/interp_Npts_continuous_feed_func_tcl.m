function [did] = interp_Npts_continuous_feed_func_tcl(fn, fnz, sid, A, dtmin, basename, basenameout, fileindx)
...
outfile = [ basenameout '_cont_feed_' num2str(fileindx)  '_traj.nc'];
outpath = '../image_code/';
...
dnS = datenum(2023,10,31,10,54,0) + 5/24;  %% local EDT time to GMT
dnE = datenum(2023,10,31,19,36,0) + 5/24;  %% past time when pumping stops. see Tf variable below
...
Tf = 2*60 + 42;  %% feed duration in minutes
Nfinc = Tf/dtmin;
...
for i = 1:ntimes
	...
	if (i < Nfinc),   %%% reset points incrementally until past feed period
		if (0),
		...
		if (1),	
			X(kk+1, tfind) = Xi(tfind);
			Y(kk+1, tfind) = Yi(tfind);
			X(kk, tfind) = Xi(tfind);
			Y(kk, tfind) = Yi(tfind);
		end;
	end;	
